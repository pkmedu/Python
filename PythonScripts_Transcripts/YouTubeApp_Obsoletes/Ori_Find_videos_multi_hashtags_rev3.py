# -*- coding: utf-8 -*-
"""
Created on Mon Apr 28 23:56:50 2025
    ...
    try:
        ...
        if custom_path:
            ...
            if file_path.is_dir():
                ...
                file_path = file_path / f"c:\Links\{safe_hashtag}.csv"
        else:
            ...
            file_path = documents_path / f"c:\Links\{safe_hashtag}.csv"
...
def open_links_in_browser(videos_info, max_tabs=5):
    ...
    try:
        ...
        if len(links) > max_tabs:
            confirm = input(f"Warning: You're about to open {len(links)} tabs. Proceed? (y/n): ").strip().lower()
        ...
        print(f"🚀 Opened {len(links)} videos in browser tabs")
...
def main():
    ...
    print("=" * 50)
    print("YouTube Hashtag Video Search Tool (Enhanced Version)")
    print("=" * 50)
    ...
    try:
        ...
        max_results_input = input("How many videos would you like to fetch per hashtag? (default 10, or type 'max'): ").strip().lower()
        max_results_input = max_results_input.strip("'\"")  # Remove any accidental quotes
        
        if max_results_input == 'max':
            max_results = sys.maxsize
            print("⚠️ Searching for maximum available results (this might take a while)")
        ...
        custom_path_prompt = input("Enter a custom save location (or press Enter for default): ").strip()
        custom_path = custom_path_prompt if custom_path_prompt else None
