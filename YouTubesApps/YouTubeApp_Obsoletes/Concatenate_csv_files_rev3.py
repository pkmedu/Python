...
for row in df_unique_sorted.itertuples(index=False):
    ...
    for col_idx, value in enumerate(row):
        ...
        if col_name == 'Link' and pd.notnull(value):
            # Directly embed hyperlink into the cell
            cell.value = value
            cell.hyperlink = value  # Set the hyperlink target
            cell.font = Font(color='0000FF', underline='single')  # Blue, underlined font for hyperlinks
        else:
            cell.value = value
...
print(f"Deduplicated and sorted Excel file with clickable hyperlinks saved to: {final_excel_path}")
