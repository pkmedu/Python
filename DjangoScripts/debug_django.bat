...
REM === Check settings module ===
echo Finding settings module... >> "%LOGFILE%"
for /f "tokens=*" %%a in ('findstr /C:"DJANGO_SETTINGS_MODULE" manage.py') do (
  echo Found settings reference: %%a >> "%LOGFILE%"
)
...
REM === List project structure ===
echo Project file structure: >> "%LOGFILE%"
dir /s /b "%REPO_DIR%\*.py" | findstr /v "__pycache__" >> "%LOGFILE%"
